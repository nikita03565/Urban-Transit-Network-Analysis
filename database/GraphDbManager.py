...
class TwoTypeNodeDBManager(GraphDBManager):
    ...
    def update_db(self, city_name):
        ...
        if (
            first_nodes is None
            and first_relationships is None
            and second_nodes is None
            and second_relationships is None
        ):
...
class RoadBuildingsDbManager(TwoTypeNodeDBManager):
    ...
    def get_graph(self, city_name):
        ...
        road_geo_to_node = {
            quads.Point(float(road_node.x), float(road_node.y)): road_node for road_node in road_nodes.itertuples()
        }
